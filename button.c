...
uint8_t button_getButton() {
    ...
    if((~GPIO_PORTE_DATA_R & 0b00100000) == 32) //0x means hex; We used hex for convenience and because the TA hates on us because decimal would have been easier to read!!!!!!!!!!!!! ;P
        return 6;
    if((~GPIO_PORTE_DATA_R & 0b00010000) == 16)
        return 5;
    if((~GPIO_PORTE_DATA_R & 0b00001000) == 8)
        return 4;
    if((~GPIO_PORTE_DATA_R & 0b00000100) == 4)
        return 3;
    if((~GPIO_PORTE_DATA_R & 0b00000010) == 2)
        return 2;
    if((~GPIO_PORTE_DATA_R & 0b00000001) == 1)
        return 1;
